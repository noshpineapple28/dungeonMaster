 ...
 * messages
 */
var messaging = true; //holds whether it is time for messaging
...
function createChat() {
  var chat = (s) => {
    let demns = dmns.chat; //holds deminsions (demns) set in main.js
    ...
    s.setSender = () => {
      ...
      s.textAlign(s.LEFT, s.CENTER);
      s.textSize(20);
      s.text("as ", s.width * 0.53, s.height * 0.82);
      //dropdown box
      //TODO: add dropdown box
      if (!selectSender) {
        s.stroke(0);
        ...
        s.text(
          sender,
          s.width * 0.53 + s.textWidth("as ") + s.width * (0.3 / 2),
          s.height * 0.78 + s.height * (0.08 / 2)
        );
        s.textAlign(s.LEFT, s.BASELINE);
        //if box is clicked, turn on recipient select mode
     ...
     */
    s.wrapText = () => {
      let chatLogs = items["sending stone"].chatLog;
      let messages = [];
      messagesLength = 0;
      ...
      chatLogs.forEach((chat) => {
        let messageBlock = [];
        if (chat.senderID === id) {
          messageBlock.push(`Sent by You as "${chat.sender}" to ${chat.recipient} : `); //sent text
          ...
          messageBlock.push(`Sent by "${chat.sender}" to ${chat.recipient}: `); //sent text
          messageBlock.push(""); //holds message we are wrapping
          let textToWrap = chat.msg.split(" ");
          for (let i = 0; i < textToWrap.length; i++) {
            if (
    ...
    s.moveCanvas = () => {
      if (messaging) {
        ...
        s.cnv.position(0 - demns.width, 0);
      }
    };
  };
